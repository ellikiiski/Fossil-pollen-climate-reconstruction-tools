...
def list_instances(sites_json_file_path):
    ...
    return aplhapbetize(taxonnames)
...
def guess_label(taxonname, rules):
    ...
    for og_name, label in rules.items():
        ...
        if label.lower() in taxonname.lower():
            ...
            guess['reasoning'] = f'Has {label} as a substring. (1)'
            return guess
        
        if og_name.lower() in taxonname.lower():
            guess['guessed'] = label.upper()
            guess['reasoning'] = f'Has {og_name} (categorized as {label}) as a substring. (2)'
            return guess
        
        elif len(first_part) >= 4 and first_part.lower() in taxonname.lower():
            guess['guessed'] = label.upper()
            guess['reasoning'] = f'Has {first_part} ({og_name} categorized as {label}) as a substring. (3)'
            return guess

    return guess


def aplhapbetize(taxonname_dict):
