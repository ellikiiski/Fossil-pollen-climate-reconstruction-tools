...
Filters: functions filter_by_pollendata and filter_by_chonologies
...
def filter_sites(datasets_file_path, write_file_path, min_oldest=None, max_youngest=None, pollen_min_number=None, chrono_min_number=None):
    ...
    filtered_by_chronologies = chronology_filter(data, chrono_min_number, min_oldest, max_youngest)
    filtered_by_pollen = pollensample_filter(filtered_by_chronologies, pollen_min_number)
...
def summary(datasets_file_path, summary_file_path):
    ...
    summarized = strip_of_samples(data)
...
def pollensample_filter(data, min_number=None):
...
def chronology_filter(data, min_number=None, min_oldest=None, max_youngest=None):
    """Reads the json file in datasets_file_path and
    returns a dictionary containing all the sites,
