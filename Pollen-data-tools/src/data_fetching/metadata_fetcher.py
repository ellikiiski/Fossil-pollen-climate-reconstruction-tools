"""
Fetch the metadata from Neotoma dabase throught the API.
...
def fetch_sites(metadata_url, dataset_url, write_file_path, coordinates=None, max_searches=None):
    ...
    pollen_ids = get_siteids(metadata_url, 'pollen', max_searches)
    print(f' DONE')
    print(f' Checking for carbon dating data.....')
    chrono_ids = get_siteids(metadata_url, 'geochronologic', max_searches)
    print(f' DONE')

    # comibine the dictionaries
    combined = combine_ids(pollen_ids, chrono_ids)
    ...
    sites = get_site_infos(combined, dataset_url, coordinates)
...
def get_siteids(url, type, max_searches=None):
...
def location_check(data, loc_limits):
...
def get_site_infos(sites, base_url, loc_limits=None):
    ...
    for (siteid, dataset_ids) in sites.items():
        ...
        if response.status_code == 200:
            ...
            location_ok = True
            if loc_limits != None:
                location_ok = location_check(data, loc_limits)
